...
function calculateCartSubtotal (totalBananas, totalApples, totalOranges) {
    return totalBananas + totalApples + totalOranges;
}
...
function calculateTotal (cartSubtotal, addedShipping, addedTax) {
    return cartSubtotal + addedShipping + addedTax;
}
...
function updateAvailabilityLabel(labelElement, amountSelected, productAvailability) {
    var available = productAvailability - amountSelected;
    if (available == 0) {
        updateProdAvailabilityLabel(labelElement, "OUT OF STOCK", ALERT);
    } else if (available <= 3) {
        updateProdAvailabilityLabel(labelElement, "LIMITED SUPPLY", WARNING);
    } else {
        updateProdAvailabilityLabel(labelElement, "IN STOCK", OK);
    }
}